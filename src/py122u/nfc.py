...
from . import error, option, utils
...
class Reader:
    ...
    def read_binary_blocks(self, block_number, number_of_byte_to_read):
        ...
        Example:
            E.g. 0x00, 0x02"""
        return self.command(
            "read_binary_blocks", [block_number, number_of_byte_to_read]
        )
    ...
    def restore_value_block(self, source_block_number, target_block_number):
        ...
        Example:
            0x01, 0x02"""
        self.command("restore_value_block", [source_block_number, target_block_number])
    ...
    def led_control(self, led_state, t1, t2, number_of_repetition, link_to_buzzer):
        ...
        self.command(
            "led_control", [led_state, t1, t2, number_of_repetition, link_to_buzzer]
        )
    ...
    def set_picc_bit(self, bit: int, value: bool):
        ...
        """
        if bit < 0 or bit > 7:
            raise error.BitOutOfRange(
                f"Bit {bit} is not in the picc operating parameter"
            )
